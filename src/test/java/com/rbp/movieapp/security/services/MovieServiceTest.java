...
public class MovieServiceTest {
    ...
    void getMovieByName() {
        // Set up mock data
        List<Movie> movies = new ArrayList<>();
        movies.add(new Movie("Avatar", "Inox", 126, "Book ASAP"));
        movies.add(new Movie("Pathaan", "Natraj", 150, "Experience the action"));
        movies.add(new Movie("Wakanda", "PVR", 148, "Your mind is the scene of the crime"));
        when(movieRepository.findByMovieName("Avatar")).thenReturn(
                movies.stream().filter(m -> m.getMovieName().equals("Avatar")).collect(Collectors.toList())
        );

        // Call the method being tested
        List<Movie> result = movieService.getMovieByName("Avatar");
    ...
    void findSeats() {
        ...
        tickets.add(new Ticket("shubhangi","Avatar", "Screen 1", 2, new ArrayList<String>(List.of("a1","a2"))));
        when(ticketRepository.findSeats("Avatar", "Screen 1")).thenReturn(tickets);

        // Call the method being tested
        List<Ticket> result = movieService.findSeats("Avatar", "Screen 1");
    ...
    void testFindAvailableTickets() {
        String movieName = "Pathaan";
        String theatreName = "Theatre 1";
        List<Movie> expectedMovies = Arrays.asList(
                new Movie("Pathaan", "Action", 10, "Incredible!"),
                new Movie("Pathaan", "Action", 10, "Amazing!"),
                new Movie("Pathaan", "Action", 10, "Thrilling!")
        );
    ...
    @Test
    void testSaveTicket() {
        Ticket expectedTicket = new Ticket("shubhangi","Inception", "Screen 1", 2, new ArrayList<String>(List.of("a1","a2")));
    ...
    @Test
    void testSaveMovie() {
        Movie expectedMovie = new Movie("Avengers: Endgame", "Action", 70, "Incredible!");
    ...
    void testGetAllBookedTickets() {
        ...
        List<Ticket> expectedTickets = Arrays.asList(
                new Ticket("Shubhangi","Life of Pie", "Screen 1", 2, new ArrayList<String>(List.of("a1","a2")))
        );
    ...
    void findByMovieName() {
        ...
        List<Movie> movies = new ArrayList<>(List.of(
                new Movie("Inception", "Hailio", 40, "Legendary"),
                new Movie("Life Of Pie", "Nikolas Hall", 17, "Underwater world movie")
        ));
